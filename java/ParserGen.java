...
public class ParserGen {
  ...
  private int framRead() {
    try {
      ...
      throw new FatalError("Error reading XmlParser.frame");
    }
  }
  ...
  void OpenGen(boolean backUp) {
    try {
      File f = new File(tab.outDir, "XmlParser.java");
      if (backUp && f.exists()) {
  ...
  public void WriteXmlParser () {
    ...
    File fr = new File(tab.srcDir, "XmlParser.frame");
    if (!fr.exists()) {
      if (tab.frameDir != null) fr = new File(tab.frameDir.trim(), "XmlParser.frame");
      if (!fr.exists()) throw new FatalError("Cannot find XmlParser.frame");
    }
    try {
    ...
    CopyFramePart("/*---- begin ----*/");
    if (!tab.srcName.toLowerCase().endsWith("cocoxml.atg")) {
      gen.close();
      OpenGen(false);
    }
